...
union YYSTYPE
{
#line 172 "parser.y" /* yacc.c:352  */

		int ival;
		char *str;
	

#line 339 "y.tab.c" /* yacc.c:352  */
};
...
{
       0,   178,   178,   179,   180,   181,   184,   203,   222,   223,
     226,   229,   230,   231,   234,   237,   238,   241,   242,   243,
     244,   245,   246,   247,   253,   254,   261,   262,   263,   266,
     266,   266,   269,   269,   269,   269,   272,   272,   272,   275,
     276,   279,   279,   279,   282,   283,   284,   285,   286,   287,
     290,   290,   290,   290,   307,   311,   312,   317,   369,   370,
     373,   373,   373,   373,   404,   422,   454,   455,   459,   463,
     466,   466,   469,   469,   470,   470,   471,   472,   472,   472,
     473,   473,   473,   474,   474,   474,   475,   475,   475,   476,
     476,   476,   477,   477,   477,   478,   478,   478,   479,   479,
     479,   480,   480,   480,   481,   483,   483,   484,   484,   485,
     487,   488,   488,   489,   490
};
#endif
...
{
  "$end", "error", "$undefined", "ENTERO", "FLOTANTE", "VACIO", "ID",
  "NUM", "REAL", "MIENTRAS", "SI", "DEVOLVER", "PREPROC", "LE", "STRING",
  "IMPRIMIR", "FUNCTION", "HACER", "ARRAY", "SINO", "ESTRUCTURA",
  "STRUCT_VAR", "PARA", "GE", "EQ", "NE", "INC", "DEC", "AND", "OR", "NEQ",
  "'<'", "'>'", "'='", "UMINUS", "'+'", "'-'", "'*'", "'/'", "'('", "')'",
...
#line 422 "parser.y" /* yacc.c:1652  */
    {
			...
			if(itype!=258)
			{ printf("\nError : El índice de matriz debe ser de tipo entero : Linea %d\n",printline());errc++;}
			if(atoi((yyvsp[-2].str))<=0)
			{ printf("\nError : El índice de matriz debe ser de tipo entero > 0 : Linea %d\n",printline());errc++;}
			if(!lookup((yyvsp[-4].str)))
			{
				...
				insert_by_scope((yyvsp[-4].str),(yyvsp[-5].ival),currscope);
					if (itype==258){
						insert_index((yyvsp[-4].str),(yyvsp[-5].ival));
					}
				}
			}
			else
			{
				...
				if (itype==258) {insert_index((yyvsp[-4].str),(yyvsp[-5].ival));}
			}
		}
